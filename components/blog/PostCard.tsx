...
export function PostCard({ post }: { post: Post }) {
  ...
  return (
    <Link
      href={`/blog/${post.slug}`}
      className="group block border border-slate-200 transition-colors hover:border-mango-500 dark:border-slate-700"
    >
      {cover?.url && (
        <Image src={cover.url} alt={cover.alt ?? post.title} width={640} height={360} className="w-full object-cover" />
      )}
      <div className="p-5">
        <h2 className="font-heading text-xl font-bold text-slate-900 group-hover:text-mango-700 dark:text-slate-100 dark:group-hover:text-mango-300">
          {post.title}
        </h2>
